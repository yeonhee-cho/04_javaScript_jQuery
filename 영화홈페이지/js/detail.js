$(function () {
  ...
  /* 
    ...
    현재컴퓨터에서.현재주소위치에서.검색한다음

    .get("?뒤에 작성된 키 이름")
        키 명칭을 가져올 것이다. 키 명칭을 가져와서 키 명칭 내부 안에 작성된 값을 전달받은 id 확인이라는 변수 이름에 담아둘 것이다.
  */
...
function loadMovieDetail(전달받은id확인) {
  // id로 전달한 고유 id를 확인해서 ghibliapi 상세보기창으로 이동 가능
...
function 무비상세보기(movie) {
  ...
  /*
    ...
    // 여러 속성 동시에 사용
    $('선택자').attr('속성명','값1').attr("속성명2",'값2')
  */
  $("#moviePoster").attr("src", movie.image);
  $("#movieTitle").text(movie.title);
  $("#movieYear").text(movie.release_date + "년");
  $("#movieDirector").text(movie.producer);
  $("#movieProducer").text(movie.release_date);
  $("#movieRelease").text(movie.running_time);
  $("#movieRuntime").text(movie.description);
}
